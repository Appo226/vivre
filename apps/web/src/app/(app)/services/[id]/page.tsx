...
function CorrectionModal({ serviceId, onClose }: CorrectionModalProps): React.ReactElement {
  ...
  return (
    ...
    >
      {/* Panel — bottom sheet */}
      <div
        className="w-full bg-surface-card rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto pb-[env(safe-area-inset-bottom)]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Handle */}
        <div className="w-10 h-1 bg-surface-elevated rounded-full mx-auto mb-4" />

        <h2 id="correction-modal-title" className="text-lg font-bold text-ink mb-1">
          Signaler une erreur
        </h2>
        ...
        {submitted ? (
          ...
          <div className="text-center py-6">
            ...
            <p className="text-base font-semibold text-ink mt-4">Signalement envoyé !</p>
            <p className="text-sm text-ink-soft mt-1">Merci pour votre aide.</p>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Type de correction */}
            <fieldset>
              <legend className="text-sm font-medium text-ink mb-2">Type de problème</legend>
            ...
            {/* Description */}
            <div>
              <label htmlFor="correction-desc" className="text-sm font-medium text-ink block mb-1">
              ...
              <textarea
                ...
                rows={4}
                maxLength={500}
                className="w-full border border-border-subtle bg-surface-card text-ink rounded-xl p-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#1A6B3A] focus:border-transparent"
                required
                minLength={5}
              />
              <p className="text-xs text-ink-soft text-right mt-1">{description.length}/500</p>
            </div>
            ...
            <div className="flex gap-3">
              <button
                type="button"
                onClick={onClose}
                className="flex-1 py-3 bg-surface-elevated text-ink rounded-2xl font-medium text-sm"
              >
                Annuler
              </button>
              <button
  ...
  if (isError || !service) {
    return (
      <div className="min-h-screen bg-page flex flex-col items-center justify-center px-4">
        ...
        <p className="text-lg font-semibold text-ink">Service introuvable</p>
      ...
      <div className="min-h-screen bg-page">
        ...
        <div className="px-4 py-5 space-y-4 max-w-lg mx-auto">
          ...
          <div className="bg-surface-card rounded-2xl p-4 shadow-sm border border-border-subtle">
            <p className="text-xs font-medium text-ink-soft uppercase tracking-wide mb-1">Adresse</p>
            <p className="text-ink text-sm">{service.address}</p>
            ...
            <div className="flex gap-2 mt-3">
              <a
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-surface-elevated text-ink rounded-xl text-sm font-medium"
              >
          ...
          {(service.phone_emergency || service.phone_primary) && (
            <div className="bg-surface-card rounded-2xl p-4 shadow-sm border border-border-subtle">
              <p className="text-xs font-medium text-ink-soft uppercase tracking-wide mb-3">Contact</p>
              ...
              {service.phone_primary && (
                <a
                  ...
                  className="flex items-center gap-3 py-2.5 px-3 bg-surface-elevated border border-border-subtle rounded-xl"
                >
                  <span className="flex items-center justify-center w-8 h-8 bg-surface-elevated rounded-lg">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-ink-soft">
                      <path fillRule="evenodd" d="M1.5 4.5a3 3 0 013-3h1.372c.86 0 1.61.586 1.819 1.42l1.105 4.423a1.875 1.875 0 01-.694 1.955l-1.293.97c-.135.101-.164.249-.126.352a11.285 11.285 0 006.697 6.697c.103.038.25.009.352-.126l.97-1.293a1.875 1.875 0 011.955-.694l4.423 1.105c.834.209 1.42.959 1.42 1.82V19.5a3 3 0 01-3 3h-2.25C8.552 22.5 1.5 15.448 1.5 6.75V4.5z" clipRule="evenodd"/>
                    </svg>
                  </span>
                  <div>
                    <p className="text-xs text-ink-soft font-medium">Principal</p>
                    <p className="text-base font-semibold text-ink">{service.phone_primary}</p>
                  </div>
                </a>
              )}
            </div>
          ...
          {service.opening_hours && Object.keys(service.opening_hours).length > 0 && (
            <div className="bg-surface-card rounded-2xl p-4 shadow-sm border border-border-subtle">
              <p className="text-xs font-medium text-ink-soft uppercase tracking-wide mb-3">Horaires</p>
              <div className="space-y-1.5">
                {Object.entries(service.opening_hours).map(([day, hours]) => (
                  <div key={day} className="flex justify-between text-sm">
                    <span className="text-ink-soft font-medium">{DAYS_FR[day] ?? day}</span>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 bg-surface-card border border-border-subtle rounded-2xl text-sm text-ink-soft font-medium shadow-sm"
          >
