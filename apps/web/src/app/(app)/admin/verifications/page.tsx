...
function VerificationCard({ v, onDecide }: { v: Verification; onDecide: () => void }): React.ReactElement {
  ...
  return (
    <div className="bg-surface-card rounded-2xl shadow-sm border border-border-subtle p-4">
      ...
      {docUrl ? (
        <a href={docUrl} target="_blank" rel="noreferrer" className="inline-block mt-3 text-sm font-semibold text-[#1A6B3A] dark:text-green-300 underline">
          Voir la pièce d&apos;identité (lien valable 5 min) →
        </a>
      ) : (
        <button onClick={() => void loadDoc()} className="mt-3 text-sm font-semibold text-[#1A6B3A] dark:text-green-300 underline">
      ...
      {mode === "idle" && (
        <div className="mt-4 flex gap-2">
          ...
          <button onClick={() => setMode("reject")} className="flex-1 bg-surface-card border border-red-200 text-red-600 text-sm font-semibold py-2.5 rounded-xl">
            Rejeter
          </button>
        </div>
      )}
      ...
      {mode === "verify" && (
        <div className="mt-4 flex flex-col gap-2">
          <p className="text-xs font-semibold text-ink-soft uppercase tracking-wide">Confirmer avant d&apos;approuver</p>
          <textarea
            value={callNotes}
          ...
          <div className="flex gap-2 mt-1">
            ...
            <button onClick={() => setMode("idle")} className="px-4 text-sm font-semibold text-ink-soft">Annuler</button>
          </div>
        </div>
      )}
      ...
      <div className="px-4 md:px-8 mt-5 md:mt-8 md:max-w-5xl">
        ...
        {!loading && verifications.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-ink-soft text-sm">Aucune vérification en attente.</p>
          </div>
        )}
